...
from lng_tracker.database.models import VesselState, VesselHistory, Users
...
async def cmd_status(message: types.Message):
    ...
    text += "━━━━━━━━━━━━━━━\n"
    for v in vessels:
        text += f"📍 <b>{v.zone}</b>: <code>{v.name}</code>\n"
    ...
    try:
        ...
        async with async_session_maker() as session:
            await session.execute(
                update(Users).where(Users.telegram_id == target_id).values(is_allowed=True)
            )
            await session.commit()
